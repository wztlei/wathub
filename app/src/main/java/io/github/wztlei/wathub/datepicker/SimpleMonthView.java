...
public class SimpleMonthView extends View {
    ...
    public SimpleMonthView(Context context) {
        ...
        mRowHeight = (res.getDimensionPixelOffset(
                R.dimen.date_picker_view_animator_height) - MONTH_HEADER_SIZE) / MAX_NUM_ROWS;
    ...
    protected void initView() {
        ...
        mSelectedCirclePaint.setColor(mMonthTitleBGColor);
    ...
    public void setMonthParams(HashMap<String, Integer> params) {
        ...
        mHighlightWeek = params.containsKey(VIEW_PARAMS_HIGHLIGHT_WEEK);
    ...
    protected void onMeasure(
        ...
        setMeasuredDimension(
                MeasureSpec.getSize(widthMeasureSpec), mRowHeight * mNumRows
                        + MONTH_HEADER_SIZE);
    }
    ...
    private String getMonthAndYearString() {
        ...
        return DateUtils.formatDateRange(
                getContext(), mFormatter, millis, millis, flags,
    ...
    private void drawMonthDayLabels(Canvas canvas) {
        ...
        for (int i = 0; i < mNumDays; i++) {
            ...
            canvas.drawText(
                    mDayLabelCalendar.getDisplayName(
                            Calendar.DAY_OF_WEEK, Calendar.SHORT,
                            Locale.getDefault()).substring(0, 1).toUpperCase(Locale.getDefault()),
    ...
    protected void drawMonthNums(Canvas canvas) {
        ...
        int rowJ = j;
        int rowY = y - FULL_ROW_OFFSET;
        if (mHighlightWeek && mSelectedDay != -1) {
            // Highlight the selected week
            for (int dayNumber = 1; dayNumber <= mNumCells; dayNumber++) {
                if (mSelectedDay == dayNumber) {
                    canvas.drawRect(0, rowY, getWidth(), rowY + DAY_SELECTED_CIRCLE_SIZE * 2,
        ...
        for (int dayNumber = 1; dayNumber <= mNumCells; dayNumber++) {
            // Draw each of the day labels
            int x = (2 * j + 1) * dayWidthHalf + mPadding;
            if (!mHighlightWeek && mSelectedDay == dayNumber) {
                canvas.drawCircle(
            ...
            } else if (mHasToday && mToday == dayNumber) {
     ...
     * in a day
     */
    public CalendarDay getDayFromLocation(
            float x,
            float y) {
     ...
     * {@link SimpleMonthView.OnDayClickListener} if one is set.
     *
    ...
    private class MonthViewNodeProvider
        ...
        @Override
        protected boolean performActionForItem(
                CalendarDay item,
                int action,
                Bundle arguments) {
            switch (action) {
        ...
        @Override
        protected void populateEventForItem(
                CalendarDay item,
        ...
        @Override
        protected void populateNodeForItem(
                CalendarDay item,
        ...
        @Override
        protected CalendarDay getItemAt(
                float x,
                float y) {
        ...
        private CharSequence getItemDescription(CalendarDay item) {
            ...
            if (item.day == mSelectedDay) {
                return getContext().getString(R.string.datetimepicker_item_is_selected, date);
            }
